...
class ProcessStoppedAnnounceRequest implements \Illuminate\Contracts\Queue\ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $peer = \App\Models\Peer::where('torrent_id', '=', $this->torrent->id)->where('peer_id', $this->queries['peer_id'])->where('user_id', '=', $this->user->id)->first();
        ...
        if ($peer === null && \strtolower($this->queries['event']) === 'completed') {
            throw new \App\Exceptions\TrackerException(151);
        }
        ...
        if ($peer === null) {
            ...
            $peer = new \App\Models\Peer();
        }
        // Get history information
        $history = \App\Models\History::where('info_hash', '=', $this->queries['info_hash'])->where('user_id', '=', $this->user->id)->first();
        ...
        $this->torrent->seeders = \App\Models\Peer::where('torrent_id', '=', $this->torrent->id)->where('left', '=', '0')->count();
        $this->torrent->leechers = \App\Models\Peer::where('torrent_id', '=', $this->torrent->id)->where('left', '>', '0')->count();
        $this->torrent->save();
    }
}
