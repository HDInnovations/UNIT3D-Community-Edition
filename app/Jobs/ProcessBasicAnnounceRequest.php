...
class ProcessBasicAnnounceRequest implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if (\config('other.freeleech') == 1 || $this->torrent->free == 1 || $personalFreeleech || $group->is_freeleech == 1 || $freeleechToken) {
            $modDownloaded = 0;
        } elseif ($this->torrent->promo50 == 1) {
            // 50% counted
            $modDownloaded = $downloaded * 0.5;
            $modDownloaded = intval(round($modDownloaded));
            //dd($modDownloaded, $downloaded);
        } elseif ($this->torrent->promo25 == 1) {
            // 75% counted
            $modDownloaded = $downloaded * 0.75;
            $modDownloaded = intval(round($modDownloaded));
        } else {
