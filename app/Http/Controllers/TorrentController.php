...
class TorrentController extends Controller
{
    ...
    /**
     * 50% Freeleech A Torrent.
     *
     * @param \App\Models\Torrent $id
     *
     * @return \Illuminate\Http\RedirectResponse
     */
    public function grant50(Request $request, $id)
    {
        $user = $request->user();
        ...
        if ($torrent->promo50 == 0) {
            $torrent->promo50 = '1';

            $this->chatRepository->systemMessage(
                \sprintf('Ladies and Gents, [url=%s]%s[/url] has been granted 50%% FreeLeech! Grab It While You Can! :fire:', $torrentUrl, $torrent->name)
            );
        } else {
            $torrent->promo50 = '0';

            $this->chatRepository->systemMessage(
                \sprintf('Ladies and Gents, [url=%s]%s[/url] has been revoked of its 50%% FreeLeech! :poop:', $torrentUrl, $torrent->name)
            );
        }
        ...
        return \redirect()->route('torrent', ['id' => $torrent->id])
            ->withSuccess('Torrent Promo-50% Has Been Adjusted!');
    }

    /**
     * 25% Freeleech A Torrent.
     *
     * @param \App\Models\Torrent $id
     *
     * @return \Illuminate\Http\RedirectResponse
     */
    public function grant25(Request $request, $id)
    {
        $user = $request->user();
        ...
        if ($torrent->promo25 == 0) {
            $torrent->promo25 = '1';

            $this->chatRepository->systemMessage(
                \sprintf('Ladies and Gents, [url=%s]%s[/url] has been granted 25%% FreeLeech! Grab It While You Can! :fire:', $torrentUrl, $torrent->name)
            );
        } else {
            $torrent->promo25 = '0';

            $this->chatRepository->systemMessage(
                \sprintf('Ladies and Gents, [url=%s]%s[/url] has been revoked of its 25%% FreeLeech! :poop:', $torrentUrl, $torrent->name)
            );
        }
        ...
        return \redirect()->route('torrent', ['id' => $torrent->id])
            ->withSuccess('Torrent Promo-25% Has Been Adjusted!');
    }
