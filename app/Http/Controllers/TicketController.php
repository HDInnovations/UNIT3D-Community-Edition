...
class TicketController extends Controller
{
    ...
    final public function show(Request $request, int $id): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View
    {
        ...
        \abort_unless($user->hasPrivilegeTo('helpdesk_can_handle') || $user->id == $ticket->user_id, 403);
    ...
    final public function close(Request $request, int $id): \Illuminate\Http\RedirectResponse
    {
        $ticket = Ticket::findOrFail($id);
        $user = $request->user();
        \abort_unless($user->hasPrivilegeTo('helpdesk_can_handle') || $user->id == $ticket->user_id || $user->id == $ticket->staff_id, 403);
