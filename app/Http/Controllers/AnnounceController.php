...
class AnnounceController extends Controller
{
    ...
    private function processAnnounceJob($queries, $user, $group, $torrent): void
    {
        ...
        if ($history->updated_at !== null && $history->updated_at->timestamp > now()->subHours(2)->timestamp && $history->seeder && $queries['left'] == 0) {
        ...
        $otherSeeders = $torrent
            ->peers
            ->where('left', '=', 0)
            ->where('peer_id', '!=', $peerId)
            ->count();
        $otherLeechers = $torrent
            ->peers
            ->where('left', '>', 0)
            ->where('peer_id', '!=', $peerId)
            ->count();

        $torrent->seeders = $otherSeeders + (int) ($queries['left'] == 0 && strtolower($queries['event']) !== 'stopped');
        $torrent->leechers = $otherLeechers + (int) ($queries['left'] > 0 && strtolower($queries['event']) !== 'stopped');
