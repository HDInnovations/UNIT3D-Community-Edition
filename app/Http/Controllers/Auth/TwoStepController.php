 ...
 */
class TwoStepController extends \App\Http\Controllers\Controller
{
    use \App\Traits\TwoStep;
    private $_authCount;
    private $_authStatus;
    ...
    private function invalidCodeReturnData($errors = null)
    {
        ...
        $returnData = ['message' => \trans('auth.titleFailed'), 'authCount' => $this->_authCount, 'remainingAttempts' => $this->_remainingAttempts];
        if ($errors) {
    ...
    public function showVerification()
    {
        ...
        $data = ['user' => $this->_user, 'remainingAttempts' => $this->_remainingAttempts + 1];
        ...
        if (!$sentTimestamp) {
            ...
            if ($carbon->gt($timeAllowedToSendCode)) {
                $this->sendVerificationCodeNotification($twoStepAuth);
                $twoStepAuth->requestDate = new \Carbon\Carbon();
                $twoStepAuth->save();
            }
        }
