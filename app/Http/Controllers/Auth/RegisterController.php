...
class RegisterController extends \App\Http\Controllers\Controller
{
    ...
    public function registrationForm($code = null)
    {
        ...
        if ($code === 'null' && \config('other.invite-only') == 1 && \config('other.application_signups') == true) {
            return \redirect()->route('application.create')->withInfo(\trans('auth.allow-invite-appl'));
        }
    ...
    public function register(\Illuminate\Http\Request $request, $code = null)
    {
        ...
        if (\config('email-blacklist.enabled') == true) {
            if (\config('captcha.enabled') == false) {
                ...
                $v = \validator($request->all(), ['username' => 'required|alpha_dash|string|between:3,25|unique:users', 'password' => 'required|string|between:8,16', 'email' => 'required|string|email|max:70|blacklist|unique:users', 'captcha' => 'hiddencaptcha']);
            }
        } elseif (\config('captcha.enabled') == false) {
            $v = \validator($request->all(), ['username' => 'required|alpha_dash|string|between:3,25|unique:users', 'password' => 'required|string|between:8,16', 'email' => 'required|string|email|max:70|unique:users']);
        } else {
            $v = \validator($request->all(), ['username' => 'required|alpha_dash|string|between:3,25|unique:users', 'password' => 'required|string|between:6,16', 'email' => 'required|string|email|max:70|unique:users', 'captcha' => 'hiddencaptcha']);
        }
        if ($v->fails()) {
            return \redirect()->route('registrationForm', ['code' => $code])->withErrors($v->errors());
        }
        $user->save();
        $userPrivacy = new \App\Models\UserPrivacy();
        ...
        if ($key) {
            ...
            $key->accepted_at = new \Carbon\Carbon();
            $key->save();
        }
