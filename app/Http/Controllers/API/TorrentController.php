...
class TorrentController extends BaseController
{
    ...
    public function filter(Request $request): TorrentsResource|\Illuminate\Http\JsonResponse
    {
        ...
        $torrents = Torrent::with(['user:id,username,group_id', 'category', 'type', 'resolution'])
            ...
            ->when($request->filled('name'), static fn($query) => $query->ofName($request->name, $isRegex($request->name)))
            ->when($request->filled('description'), static fn($query) => $query->ofDescription($request->description, $isRegex($request->description)))
            ->when($request->filled('mediainfo'), static fn($query) => $query->ofMediainfo($request->mediainfo, $isRegex($request->mediainfo)))
            ->when($request->filled('uploader'), static fn($query) => $query->ofUploader($request->uploader))
            ->when($request->filled('keywords'), static fn($query) => $query->ofKeyword(\array_map('trim', explode(',', $request->keywords))))
            ->when($request->filled('startYear'), static fn($query) => $query->releasedAfterOrIn((int) $request->startYear))
            ->when($request->filled('endYear'), static fn($query) => $query->releasedBeforeOrIn((int) $request->endYear))
            ->when($request->filled('categories'), static fn($query) => $query->ofCategory($request->categories))
            ->when($request->filled('types'), static fn($query) => $query->ofType($request->types))
            ->when($request->filled('resolutions'), static fn($query) => $query->ofResolution($request->resolutions))
            ->when($request->filled('genres'), static fn($query) => $query->ofGenre($request->genres))
            ->when($request->filled('tmdbId'), static fn($query) => $query->ofTmdb((int) $request->tmdbId))
            ->when($request->filled('imdbId'), static fn($query) => $query->ofImdb((int) $request->imdbId))
            ->when($request->filled('tvdbId'), static fn($query) => $query->ofTvdb((int) $request->tvdbId))
            ->when($request->filled('malId'), static fn($query) => $query->ofMal((int) $request->malId))
            ->when($request->filled('playlistId'), static fn($query) => $query->ofPlaylist((int) $request->playlistId))
            ->when($request->filled('collectionId'), static fn($query) => $query->ofCollection((int) $request->collectionId))
            ->when($request->filled('free'), static fn($query) => $query->ofFreeleech($request->free))
            ->when($request->filled('doubleup'), static fn($query) => $query->doubleup())
            ->when($request->filled('featured'), static fn($query) => $query->featured())
            ->when($request->filled('stream'), static fn($query) => $query->streamOptimized())
            ->when($request->filled('sd'), static fn($query) => $query->sd())
            ->when($request->filled('highspeed'), static fn($query) => $query->highspeed())
            ->when($request->filled('internal'), static fn($query) => $query->internal())
            ->when($request->filled('personalRelease'), static fn($query) => $query->personalRelease())
            ->when($request->filled('alive'), static fn($query) => $query->alive())
            ->when($request->filled('dying'), static fn($query) => $query->dying())
            ->when($request->filled('dead'), static fn($query) => $query->dead())
            ->when($request->filled('file_name'), static fn($query) => $query->ofFilename($request->file_name))
            ->when($request->filled('seasonNumber'), static fn($query) => $query->ofSeason((int) $request->seasonNumber))
            ->when($request->filled('episodeNumber'), static fn($query) => $query->ofEpisode((int) $request->episodeNumber))
            ->latest('sticky')
