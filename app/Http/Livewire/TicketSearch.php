    ...
    final public function getTicketsProperty(): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        if ($this->user->group->is_modo) {
            return Ticket::query()
                ...
                ->when(!$this->show, static fn($query) => $query->whereNull('closed_at'))
                ->when($this->show, static fn($query) => $query->whereNotNull('closed_at')->orWhereNull('closed_at'))
        ...
        return Ticket::query()
            ...
            ->when(!$this->show, static fn($query) => $query->whereNull('closed_at'))
            ->when($this->show, static fn($query) => $query->whereNotNull('closed_at'))
            ->when($this->search, fn ($query) => $query->where('subject', 'LIKE', '%'.$this->search.'%'))
            ->orderBy($this->sortField, $this->sortDirection)
            ->paginate($this->perPage);
    }
