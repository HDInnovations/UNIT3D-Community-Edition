...
class TorrentSearch extends Component
{
    ...
    final public function getTorrentsProperty(): \Illuminate\Contracts\Pagination\LengthAwarePaginator
    {
        ...
        $torrents = Torrent::select(['id', 'name', 'size', 'seeders', 'leechers', 'times_completed',
            'user_id', 'release_year', 'region_id', 'distributor_id', 'tmdb', 'imdb', 'tvdb', 'mal',
            'category_id', 'type_id', 'resolution_id', 'free', 'doubleup', 'featured', 'internal',
            'personal_release', 'stream', 'sd', 'highspeed', 'sticky', 'bumped_at', 'created_at',
            'fl_until', 'du_until', 'status', 'anon'])
            ...
            ->when($this->description !== '', fn ($query) => $query->addSelect('description')->ofDescription($this->description, $isRegex($this->description)))
            ->when($this->mediainfo !== '', fn ($query) => $query->addSelect('mediainfo')->ofMediainfo($this->mediainfo, $isRegex($this->mediainfo)))
