                ...
                <div class="col-sm-12 movie-list" style=" margin: 0;">
                    <div class="row">
                        ...
                        <section class="col-sm-3">
                            <img src="{{ isset($details->still) ? \tmdb_image('cast_big', $details->still) : 'https://via.placeholder.com/300x450' }}"
                        ...
                        <section class="col-sm-9">
                            ...
                            <dl class="dl-horizontal">
                                <dt><i class="fa fa-heart text-green" aria-hidden="true"></i> @lang('mediahub.born') </dt>
                                <dd>
                                    {{ $details->birthday ?? '' }}
                                    In {{ $details->place_of_birth ?? '' }}
                                </dd>
                                <br>
                                <dt><i class="fa fa-film text-green" aria-hidden="true"></i> @lang('mediahub.movie-credits')
                                </dt>
                                <dd>{{ $credits->movie->count() ?? '0' }}</dd>
                                <dt><i class="fa fa-step-backward text-green" aria-hidden="true"></i>
                                    @lang('mediahub.first-seen') </dt>
                                <dd>In <a
                                        href="{{ route('mediahub.movies.show', ['id' => $credits->movie->first()->id ?? '0']) }}">{{ $credits->movie->first()->title ?? 'N/A' }}</a>
                                </dd>
                                <dt><i class="fa fa-step-forward text-green" aria-hidden="true"></i>
                                    @lang('mediahub.latest-project') </dt>
                                <dd>Last in <a
                                        href="{{ route('mediahub.movies.show', ['id' => $credits->movie->last()->id ?? '0']) }}">{{ $credits->movie->last()->title ?? 'N/A' }}
                                    </a></dd>
                                <br>
                                <dt><i class="fa fa-tv-retro text-green" aria-hidden="true"></i>
                                    @lang('mediahub.tv-credits') </dt>
                                <dd>{{ $credits->tv->count() ?? '0' }}</dd>
                                <dt><i class="fa fa-step-backward text-green" aria-hidden="true"></i>
                                    @lang('mediahub.first-seen') </dt>
                                <dd>In <a
                                        href="{{ route('mediahub.shows.show', ['id' => $credits->tv->first()->id ?? '0']) }}">{{ $credits->tv->first()->name ?? 'N/A' }}</a>
                                </dd>
                                <dt><i class="fa fa-step-forward text-green" aria-hidden="true"></i>
                                    @lang('mediahub.latest-project') </dt>
                                <dd>Last in <a
                                        href="{{ route('mediahub.shows.show', ['id' => $credits->tv->last()->id ?? '0']) }}">{{ $credits->tv->last()->name ?? 'N/A' }}
                                    </a></dd>
                            </dl>
                ...
                <tbody>
                    @if (!empty($credits->movie))
                        @if (count($credits->movie) <= 0)
                            <div class="row">
                                <div class="col-md-12 text-center">
                                    <h1 class="text-blue">
                                        <i class="{{ config('other.font-awesome') }} fa-frown text-blue"></i>No Data
                                        Found!
                                    </h1>
                                </div>
                            </div>
                        ...
                        @foreach ($credits->movie as $movie)
                            <tr>
                                <td class="col-sm-1">
                                    <img src="{{ isset($movie->poster) ? \tmdb_image('poster_small', $movie->poster) : 'https://via.placeholder.com/90x135' }}"
                ...
                <tbody>
                    @if (!empty($credits->tv))
                        @if (count($credits->tv) <= 0)
                            <div class="row">
                                <div class="col-md-12 text-center">
                                    <h1 class="text-blue">
                                        <i
                                            class="{{ config('other.font-awesome') }} fa-frown text-blue"></i>@lang('mediahub.no-data')
                                    </h1>
                                </div>
                            </div>
                        @endif
                        @foreach ($credits->tv as $show)
                            <tr>
                                <td class="col-sm-1">
                                    <img src="{{ isset($show->poster) ? \tmdb_image('poster_small', $show->poster) : 'https://via.placeholder.com/90x135' }}"
                                        alt="{{ $show->name }}" class="img-responsive">
                                </td>
                                <td class="col-sm-5">
                                    <i class="fa fa-tv-retro text-purple" aria-hidden="true"></i> <strong>
                                        @lang('mediahub.show')</strong><br>
                                    <i class="fa fa-eye text-green" aria-hidden="true"></i> <a
