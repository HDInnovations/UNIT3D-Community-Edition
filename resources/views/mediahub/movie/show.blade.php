                ...
                <table class="table table-condensed table-bordered table-striped">
                    @foreach (App\Models\Type::all()->sortBy('position') as $type)
                        ...
                        @foreach (App\Models\Resolution::all()->sortBy('position') as $resolution)
                            ...
                            @endif
                            <tbody>
                                @foreach ($movie->torrents->where('type_id', '=', $type->id)->where('resolution_id', '=', $resolution->id)->sortByDesc('created_at')
        as $torrent)
                                    @if ($torrent->sticky == 1)
                                        <tr class="success">
                                        @else
                                    ...
                                    <td>
                                        ...
                                        @if (config('torrent.download_check_page') == 1)
                                            <a href="{{ route('download_check', ['id' => $torrent->id]) }}">
                                                <button class="btn btn-primary btn-circle" type="button"
                                                    data-toggle="tooltip" data-original-title="@lang('common.download')">
                                        ...
                                        @php
                                            $history = \App\Models\History::where('user_id', '=', $user->id)
                                        ...
                                        <span class="badge-extra text-green">
                                            <i class="{{ config('other.font-awesome') }} fa-comment"
                                                data-toggle="tooltip" data-original-title="@lang('common.comments')"></i>
                                        ...
                                        @if ($torrent->internal == 1)
                                            <span class='badge-extra'>
                                                <i class='{{ config('other.font-awesome') }} fa-magic'
                                                    ...
                                                    torrent.internal-release')' style="color: #baaf92;"></i>
                                            </span>
                                        @endif
                                        ...
                                        @php
                                            $freeleech_token = \App\Models\FreeleechToken::where('user_id', '=', $user->id)
