...
@section('content')
    <div class="container">
        ...
        <div class="block">
            ...
            <div class="row">
                <div class="col-md-12">
                    <p class="text-purple"><strong><i class="{{ config('other.font-awesome') }} fa-coins"></i>
                            @lang('stat.top-bankers')</strong>
                        (@lang('bon.bon'))</p>
                                        ...
                                        @if ($b->private_profile == 1)
                                            <span class="badge-user text-bold"><span class="text-orange"><i
                                                ...
                                                @if (auth()->user()->id == $b->id || auth()->user()->group->is_modo)
                                                    <a
                                                        href="{{ route('users.show', ['username' => $b->username]) }}">({{ $b->username }}</a>
                                            </span>
                                        @endif
                                    @else
