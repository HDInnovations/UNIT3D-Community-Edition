            ...
            <div class="col-md-12">
                <form action="{{route('poster_search')}}" class="form-horizontal" method="get">
                    ...
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="form-group">
                                <label class="pull-left control-label" for="genre">{{ trans('torrent.category') }}
                                    :</label>
                        ...
                        <div class="col-lg-3 col-lg-offset-1">
                            <div class="form-group">
                                <label class="pull-left control-label" for="order">{{ trans('common.order-by') }}
                                    :</label>
                ...
                <div style="float:left;">
                    ...
                    <a href="{{ route('categories') }}" class="btn btn-xs btn-primary"><em
                                class="icon fa fa-film"></em> {{ trans('torrent.categories') }}</a>
                    <a href="{{ route('catalogs') }}" class="btn btn-xs btn-primary"><em
                                class="icon fa fa-film"></em> {{ trans('torrent.catalogs') }}</a>
                </div>
                <div style="float:right;">
                    <strong>{{ trans('common.view') }}:</strong>
                    <a href="{{ route('torrents') }}" class="btn btn-xs btn-primary"><i
                                class="fa fa-list"></i> {{ trans('common.lists') }}</a>
                    <a href="{{ route('poster') }}" class="btn btn-xs btn-info"><i
                                class="fa fa-image"></i> {{ trans('torrent.posters') }}</a>
                </div>
                <br>
            </div>
        </div>
                    ...
                    <div class="image-box text-center mb-20">
                        <div class="overlay-container">
                            ...
                            <div class="overlay-top">
                                <div class="text">
                                    <h2>
                                        <a data-id="{{ $t->id }}" data-slug="{{ $t->slug }}"
                                           href="{{ route('torrent', array('slug' => $t->slug, 'id' => $t->id)) }}">{{ $t->name }}</a>
                                    </h2>
                                </div>
                            </div>
                            <div class="overlay-bottom">
                                <div class="links">
                                    ...
                                    <ul class="list-unstyled margin-clear">
                                        <li><i class="fa fa-database"></i> {{ trans('torrent.size') }}
                                            : {{ $t->getSize() }}</li>
                                        <li><i class="fa fa-arrow-up"></i> {{ trans('torrent.seeds') }}
                                            : {{ $t->seeders }}</li>
                                        <li><i class="fa fa-arrow-down"></i> {{ trans('torrent.leeches') }}
                                            : {{ $t->leechers }}</li>
                                        <li><i class="fa fa-check"></i> {{ trans('torrent.completed') }}
                                            : {{ $t->times_completed }}</li>
                                        <li>
                                            <a rel="nofollow"
                                               href="https://anon.to?http://www.imdb.com/title/tt{{ $t->imdb }}"
                                               title="IMDB" target="_blank"><span class='label label-success'>{{ trans('common.view') }}
                                                    IMDB</span></a>
                                        </li>
                                    </ul>
                                </div>
