...
@section('content')
    <div id="torrent-page">
                ...
                <div class="button-block">
                    ...
                    @if ($playlists->count() > 0)
                        <button class="btn btn-sm btn-primary" data-toggle="modal"
                                data-target="#modal_playlist_torrent">
                    ...
                    @if ($current = $user->history->where('info_hash', $torrent->info_hash)->first())
                        @if ($current->seeder == 0 && $current->active == 1 && $torrent->seeders <= 2)
                            <a href="{{ route('reseed', ['id' => $torrent->id]) }}" role="button"
        ...
        <div class="meta-general box container">
                            ...
                            <td>
                                <div style="margin: 4px 0">
                                    <div class="torrent-format" style="display: inline-block">
                                            <span class="torrent-category badge-extra text-info text-bold"
                                                  style="line-height: 14px">
                                                {{ $torrent->category->name }}
                                            </span>
                                        <span class="torrent-resolution badge-extra text-info text-bold"
                                              style="line-height: 14px">
                                                {{ $torrent->resolution->name ?? 'No Res' }}
                                            </span>
                                        <span class="torrent-type badge-extra text-info text-bold"
                                              style="line-height: 14px">
                                                {{ $torrent->type->name }}
                                            </span>
                                        <span
                                    ...
                                    <div class="torrent-discounts" style="display: inline-block">
                                        @if ($torrent->featured == '0')
                                            @if ($freeleech_token || $user->hasPrivilegeTo('user_special_freeleech') || $personal_freeleech || $torrent->free == '1' || config('other.freeleech') == '1' || $torrent->doubleup == '1' || $user->hasPrivilegeTo('user_special_double_upload') || config('other.doubleup') == '1')
                                                @if ($freeleech_token || $user->hasPrivilegeTo('user_special_freeleech') || $personal_freeleech || $torrent->free == '1' || config('other.freeleech') == '1')
                                            ...
                                            @else
                                                <span class="text-danger badge-extra" data-toggle="tooltip"
                                                      title="@lang('torrent.no-discounts')">
                                                        <i class="{{ config('other.font-awesome') }} fa-frown"></i>
                                                    </span>
                                            @endif
                                        @else
                                            <span class="badge-extra" data-toggle="tooltip" data-html="true"
                                                  title='@lang("torrent.featured-until") {{ $featured->created_at->addDay(7)->toFormattedDateString() }} ({{ $featured->created_at->addDay(7)->diffForHumans() }}!) {!! trans("torrent.featured-desc") !!}'>
                                                    <i class="{{ config('other.font-awesome') }} fa-certificate text-orange"></i>
                                                </span>
                                        @endif

                                        @if ($torrent->internal == '1')
                                            <span class="badge-extra" data-toggle="tooltip"
                                        ...
                                        @if ($torrent->personal_release == '1')
                                            <span class="badge-extra" data-toggle="tooltip"
                                                  title="@lang('torrent.personal-release')">
                                                    <i class="{{ config('other.font-awesome') }} fa-user-plus text-green"
                                                       style="color: #865be9"></i>
                                                </span>
                                        @endif
                                    ...
                                    <div class="torrent-uploaded pull-right">
                                        ...
                                        @if ($torrent->anon == 1)
                                            <span class="badge-user text-orange text-bold">{{ strtoupper(trans('common.anonymous')) }}
                                                @if (auth()->user()->id == $uploader->id || $user->hasPrivilegeTo('users_view_private'))
                                        ...
                                        @else
                                            <a href="{{ route('users.show', ['username' => $uploader->username]) }}">
                                                    <span class="badge-user text-bold"
                                                          style="color:{{ $uploader->primaryRole->color }}; background-image:{{ $uploader->primaryRole->effect }};">
                                        ...
                                        @if ($torrent->anon !== 1 && $uploader->private_profile !== 1)
                                            @if (auth()->user()->isFollowing($uploader->id))
                                                <form class="form-inline" style="line-height: 0; display: inline-block;"
                                                      role="form"
                                                    ...
                                                    <div class="form-group">
                                                        <button type="submit"
                                                                id="delete-follow-{{ $uploader->target_id }}"
                                        ...
                                        <span class="torrent-uploaded-time" data-toggle="tooltip"
                            ...
                            <tr class="info torrent-featured">
                                ...
                                <td>
                                    <span class="badge-user text-bold text-pink"
                                          style="background-image:url(https://i.imgur.com/F0UCb7A.gif);">
            ...
            @if (auth()->user()->hasPrivilegeTo('torrent_can_moderate') || auth()->user()->id === $uploader->id || auth()->user()->hasPrivilegeTo('torrent_can_internal'))
                            ...
                            <tr>
                                <td style="display: flex; justify-content: space-around; padding: 10px 0">
                                    @if (auth()->user()->hasPrivilegeTo('torrent_can_update') || auth()->user()->id === $uploader->id)
                                        <div class="torrent-editing-controls">
                                            <a class="btn btn-warning btn-xs"
                                               href="{{ route('edit_form', ['id' => $torrent->id]) }}" role="button">
                                    ...
                                    @if (auth()->user()->hasPrivilegeTo('torrent_can_doubleupload'))
                                            ...
                                            @endif
                                    @endif
                                    @if (auth()->user()->hasPrivilegeTo('torrent_can_sticky'))
                                            @if ($torrent->sticky == 0)
                                    ...
                                    @if (auth()->user()->hasPrivilegeTo('torrent_can_moderate'))
                                        <div class="torrent-moderation-controls">
                                            <a href="{{ route('staff.moderation.approve', ['id' => $torrent->id]) }}"
                                               role='button'
            ...
            @if (auth()->user()->hasPrivilegeTo('user_special_staff'))
                            ...
                            <tr>
                                <td>
                                    <div class="panel-body">
                                        <div class="slidingDiv2">
                                            ...
                                            <div class="mediainfo-main" style="width: 100%; display:table;">
                                                <div class="mediainfo-general"
                                                    ...
                                                    <div class="text-bold">@joypixels(':information_source:') General:
                                                    </div>
                                                    <div>
                                                ...
                                                <div class="mediainfo-video"
                                                     style="width: 30%; display:table-cell; text-align: left;">
                                                    ...
                                                    @if ($mediaInfo !== null && isset($mediaInfo['video']))
                                                        @foreach ($mediaInfo['video'] as $key => $videoElement)
                                                            ...
                                                            <div>
                                                                ...
                                                                x {{ $videoElement['height'] ?? trans('common.unknown') }}
                                                            </div>
                                                            ...
                                                            @endif
                                                            @if(count($mediaInfo['video']) > 1)
                                                                <div style="border-top: 1px solid #444444; padding-top: 5px; margin-top: 5px; width: 75%;"></div> @endif
                                                        @endforeach
                                                    @endif
                                                </div>
                                                <div class="mediainfo-audio"
                                                     style="width: 50%; display:table-cell; text-align: left;">
                                                    <div class="text-bold">@joypixels(':loud_sound:') Audio Tracks:
                                                    </div>
                                        ...
                                        <div class="torrent-mediainfo-dump" style="opacity: 1; display: none;"
                                             x-show="show">
            ...
            <div class="panel panel-chat shoutbox torrent-description">
                <div class="panel-heading">
                    <h4><i class="{{ config("other.font-awesome") }} fa-sticky-note"></i> @lang('common.description')
                    </h4>
                </div>
                <div class="table-responsive">
                    <table class="table table-condensed table-bordered table-striped">
                                ...
                                <div class="col-md-7">
                                    <form role="form" method="POST"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <label>
                                                <input type="number" name="tip" value="0" placeholder="0"
                                                       class="form-control"
                                                       style="width: 80%;">
                                            </label>
                                            <button type="submit"
                                            ...
                                            <a href="{{ route('torrents.similar', ['category_id' => $torrent->category_id, 'tmdb' => $recommendation->recommendation_movie_id ?? $recommendation->recommendation_tv_id]) }}">
                                                <div>
                                                    @if(isset($recommendation->poster))
                                                        <img class="backdrop"
                                                             src="{{ \tmdb_image('poster_big', $recommendation->poster) }}">
                                                    @else
