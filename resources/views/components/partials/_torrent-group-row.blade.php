<td class="torrent-search--grouped__overview">
    <div>
            ...
            <a href="{{ route('torrent', ['id' => $torrent->id]) }}">
                @switch ($media->meta)
                    ...
                    @case('tv')
                        ...
                        {{ \preg_replace('/^.*( S\d{2,4}(?:-?E\d{2,4})*? | \d{4}(?:-\d{2}){1,2} | '.(substr($media->release_date ?? '0', 0, 4) - 1).' | '.substr($media->release_date ?? '0', 0, 4).' | '.(substr($media->release_date ?? '0', 0, 4) + 1).' )/i', '', $torrent->name) }}
                        @break
                @endswitch
            </a>
        </h3>
        <span class="torrent-search--grouped__flags">
            ...
            @endif
            @if (auth()->user()->group->is_freeleech == 1)
                <i
            ...
            @if (config('other.doubleup') == 1)
                ...
                ></i>
            @endif
            @if (auth()->user()->group->is_double_upload == 1)
                <i
