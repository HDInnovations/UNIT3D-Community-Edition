...
@section('content')
    <div class="topic container">
        ...
        <div class="topic-info">
            ...
            <span class='label label-primary'>{{ $topic->num_post - 1 }}
                {{ strtolower(trans('forum.replies')) }}</span>
                        ...
                        <div class="profil">
                            ...
                            <aside class="col-md-2 post-info">
                                @if ($p->user->image != null)
                                    <img src="{{ url('files/img/' . $p->user->image) }}"
                                        alt="{{ $p->user->username }}" class="img-thumbnail post-info-image">
                                @else
                                    ...
                                    <span class="badge-user text-bold">
                                        ...
                                        @if ($p->user->isOnline())
                                            <i class="{{ config('other.font-awesome') }} fa-circle text-green"
                                                data-toggle="tooltip" data-original-title="Online"></i>
                                        @else
                                ...
                                @endif
                                <p><span class="badge-user text-bold">Joined:
                                        {{ date('d M Y', $p->user->created_at->getTimestamp()) }}</span></p>

                                <p>
                                    @if ($p->user->topics && $p->user->topics->count() > 0)
                                        <span class="badge-user text-bold">
                                            <a href="{{ route('user_topics', ['username' => $p->user->username]) }}"
                                                class="post-info-username">{{ $p->user->topics->count() }}
                                                @lang('forum.topics')</a>
                                        </span>
                                    @endif
                                    @if ($p->user->posts && $p->user->posts->count() > 0)
                                        <span class="badge-user text-bold">
                                            <a href="{{ route('user_posts', ['username' => $p->user->username]) }}"
                                                class="post-info-username">{{ $p->user->posts->count() }}
                                                @lang('forum.posts')</a>
                                        </span>
                                    @endif
                                </p>
                                ...
                                <span class="inline">
                                    ...
                                    @if (auth()->user()->group->is_modo || $p->user_id === auth()->user()->id)
                                        <a
                                            href="{{ route('forum_post_edit_form', ['id' => $topic->id, 'postId' => $p->id]) }}"><button
                                ...
                                <div id="forumTip{{ $p->id }}" class="text-center">
                                    ...
                                    <div id="forumTip" route="{{ route('tip_poster') }}"
                                        ...
                                        <a class="forumTip" href="#/" post="{{ $p->id }}"
                                            user="{{ $p->user->id }}">@lang('forum.tip-this-post')</a>
                                    </div>
                                </div>
                            </div>
