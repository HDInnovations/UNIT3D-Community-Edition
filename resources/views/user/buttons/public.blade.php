        ...
        @if (auth()->user()->id != $user->id)
            @if (auth()->user()->isFollowing($user->id))
                <a href="{{ route('follow.destroy', ['username' => $user->username]) }}"
                    id="delete-follow-{{ $user->target_id }}" class="btn btn-sm btn-danger" title="@lang('user.unfollow')"><i
                        class="{{ config('other.font-awesome') }} fa-user"></i> @lang('user.unfollow')</a>
            @else
                <a href="{{ route('follow.store', ['username' => $user->username]) }}" id="follow-user-{{ $user->id }}"
                    class="btn btn-sm btn-warning" title="@lang('user.follow')"><i
