            ...
            <table class="table table-condensed table-striped table-bordered" id="torrent-list-table">
                ...
                <tbody>
                    @foreach ($torrents as $torrent)
                        @php
                            $history = \App\Models\History::where('info_hash', '=', $torrent->info_hash)
                        ...
                        @if ($torrent->category->tv_meta)
                            @if ($torrent->tmdb || $torrent->tmdb != 0)
                                @php
                                    $meta = App\Models\Tv::with('genres')
                                        ->where('id', '=', $torrent->tmdb)
                                        ->first();
                                @endphp
                            @endif
                        @endif
                        @if ($torrent->category->movie_meta)
                            @if ($torrent->tmdb || $torrent->tmdb != 0)
                                @php
                                    $meta = App\Models\Movie::with('genres')
                                        ->where('id', '=', $torrent->tmdb)
                                        ->first();
                                @endphp
                            @endif
                        ...
                        <tr>
                            ...
                            <td style="vertical-align: middle;" x-data="{ tooltip: false }">
                                <a x-on:mouseenter="tooltip = true" x-on:mouseleave="tooltip = false"
                                    class="view-torrent" style="font-size: 16px; font-weight: normal;"
                                ...
                                <div style="z-index: 5; position: absolute;" x-cloak
                            ...
                            <td style="vertical-align: middle;">
                                @php
                                    $resurrected = DB::table('graveyard')
                                ...
                                @if (!$resurrected)
                                    ...
                                    <div class="modal fade" id="resurrect-{{ $torrent->id }}" tabindex="-1"
                                        role="dialog" aria-labelledby="resurrect">
                                                    ...
                                                    <div class="text-center">
                                                        <p>{!! trans('graveyard.howto-desc1', ['name' => $torrent->name]) !!}
                                                            ...
                                                            <span class="badge-user text-bold text-pink"
                                                                style="background-image:url(/img/sparkels.gif);">
                                                                {{ config('graveyard.reward') }}
                                                                @lang('torrent.freeleech') Token(s)!
                                                            </span>
                                                        </p>
                                                    </div>
                                                </div>
