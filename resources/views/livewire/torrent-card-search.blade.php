					...
					<div class="card is-torrent">
						...
						<div class="card_body">
							<div class="body_poster">
								@if ($torrent->category->movie_meta || $torrent->category->tv_meta)
									<img src="{{ isset($meta->poster) ? \tmdb_image('poster_mid', $meta->poster) : 'https://via.placeholder.com/200x300' }}"
						...
						<div class="card_footer">
								...
								@else
									<a href="{{ route('users.show', ['username' => $torrent->user->username]) }}">
										<span class="badge-user text-bold"
											...
											<i class="{{ $torrent->user->group->icon }}" data-toggle="tooltip" data-original-title="{{ $torrent->user->group->name }}"></i>
											{{ $torrent->user->username }}
										</span>
									</a>
								@endif
							...
							<span class="badge-user text-bold" style="float: right;">
								...
								@if($torrent->category->movie_meta || $torrent->category->tv_meta)
									{{ round($meta->vote_average ?? 0) }}/10 ({{ $meta->vote_count ?? 0 }} @lang('torrent.votes'))
								@endif
								@if($torrent->category->game_meta)
