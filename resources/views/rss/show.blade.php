        ...
        @if($torrents)
            @foreach($torrents as $data)
                <item>
                    ...
                    <description>{{ $data->category->name }} / {{ $data->type }} / {{ $data->resolution }} / {{ $data->getSize() }} @if($data->freeleech === 1) / Double Upload! @endif @if($data->doubleup === 1) / Freeleech! @endif</description>
