                ...
                <div class="row mb-10">
                    <div class="col-sm-12">
                        <div class="pull-right">
                            <button class="btn btn-xs btn-danger" data-toggle="modal" data-target="#modal_request_report"><i
                                    class="{{ config('other.font-awesome') }} fa-eye"></i>
                                @lang('request.report')</button>
                                ...
                                @endif @if ($user->group->is_modo && $torrentRequest->filled_hash != null)
                                    ...
                                    @endif @if ($user->group->is_modo || ($torrentRequest->user->id == $user->id && $torrentRequest->filled_hash == null))
                                        <a class="btn btn-warning btn-xs"
                                            ...
                                            role="button"><i class="{{ config('other.font-awesome') }} fa-edit"
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <i class="{{ $torrentRequest->category->icon }} torrent-icon torrent-icon-small"
                                        data-toggle="tooltip"
                                        data-original-title="{{ $torrentRequest->category->name }} Torrent"></i>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <strong>{{ $torrentRequest->bounty }}</strong> @lang('bon.bon')
                                    {{ strtolower(trans('request.reward-from')) }}
                                    <strong>{{ $torrentRequest->requestBounty->count() }}</strong>
                                    {{ strtolower(trans('request.voters')) }}
                                </td>
                            </tr>
                            <tr>
                                <td>
                                ...
                                <td>
                                    @if ($torrentRequest->anon == 0)
                                        <span class="badge-user">
                                            <a
                                                href="{{ route('users.show', ['username' => $torrentRequest->user->username]) }}">
                                ...
                                <td>
                                    @if ($torrentRequest->claimed == null && $torrentRequest->filled_hash == null)
                                        ...
                                        @if ($torrentRequest->category->movie_meta || $torrentRequest->category->tv_meta)
                                            <a href="{{ route('upload_form', ['category_id' => $torrentRequest->category_id, 'title' => $meta->title ?? ' ', 'imdb' => $meta->imdb ?? 0, 'tmdb' => $meta->tmdb ?? 0]) }}"
                                                class="btn btn-xs btn-success"> @lang('common.upload')
                                                {{ $meta->title ?? '' }}
                                            </a>
                                        @endif
                                    ...
                                    @else
                                        @if ($torrentRequestClaim->anon == 0)
                                            <span class="badge-user">
                                                <a
                                                    href="{{ route('users.show', ['username' => $torrentRequestClaim->username]) }}">
                            ...
                            @if ($torrentRequest->filled_hash != null && $torrentRequest->approved_by != null)
                                <tr>
                                    ...
                                    <td>
                                        @if ($torrentRequest->filled_anon == 0)
                                            <span class="badge-user">
                                                <a
                                                    href="{{ route('users.show', ['username' => $torrentRequest->FillUser->username]) }}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a
                                            href="{{ route('torrent', ['id' => $torrentRequest->torrent->id]) }}">{{ $torrentRequest->torrent->name }}</a>
                                    </td>
                                </tr>
                            @endif
                                        ...
                                        <tr>
                                            <td>
                                                @if ($voter->anon == 0)
                                                    <span class="badge-user">
                                                        <a
                                                            href="{{ route('users.show', ['username' => $voter->user->username]) }}">
                                                            {{ $voter->user->username }}
                                                        </a>
                                                    </span>
                                                @else
                                    ...
                                    @if (count($comments) == 0)
                                        <div class="text-center">
                                            <h4 class="text-bold text-danger"><i
                                                    class="{{ config('other.font-awesome') }} fa-frown"></i>
                                                @lang('common.no-comments')!
                                            </h4>
                                        </div>
                                            ...
                                            <li class="media" style="border-left: 5px solid #01bc8c;">
                                                <div class="media-body">
                                                    ...
                                                    <strong><a
                                                            ...
                                                            style="color:{{ $comment->user->group->color }};"><span><i
                                                                    class="{{ $comment->user->group->icon }}"></i>
                                                                {{ $comment->user->username }}</span></a></strong>
                                        @endif
                                        <span class="text-muted"><small><em>{{ $comment->created_at->toDayDateTimeString() }}
                                                    ({{ $comment->created_at->diffForHumans() }})</em></small></span>
