                            ...
                            @if (auth()->user()->id == $playlist->user_id || auth()->user()->group->is_modo)
                                ...
                                <form action="{{ route('playlists.destroy', ['id' => $playlist->id]) }}" method="POST"
                                    ...
                                    <a href="{{ route('playlists.edit', ['id' => $playlist->id]) }}"
                                        ...
                                        @lang('playlist.edit-playlist')
                                    </a>
                                    <button type="submit" class="btn btn-danger pull-right">
                                        <i class="{{ config('other.font-awesome') }} fa-trash"></i>
                                        @lang('playlist.delete-playlist')
                                    </button>
                                </form>
                            @endif
                        </div>
                ...
                @foreach ($torrents as $t)
                    @if ($t->torrent->category->tv_meta)
                        @if ($t->torrent->tmdb || $t->torrent->tmdb != 0)
                            @php
                                $meta = App\Models\Tv::with('genres', 'networks', 'seasons')
                                    ->where('id', '=', $t->torrent->tmdb)
                                    ->first();
                            @endphp
                        @endif
                    @endif
                    @if ($t->torrent->category->movie_meta)
                        @if ($t->torrent->tmdb || $t->torrent->tmdb != 0)
                            @php
                                $meta = App\Models\Movie::with('genres', 'cast', 'companies', 'collection')
                                    ->where('id', '=', $t->torrent->tmdb)
                                    ->first();
                            @endphp
                        @endif
                            ...
                            <div class="card_head">
                                ...
                                @if (auth()->user()->id == $playlist->user_id || auth()->user()->group->is_modo)
                                    <form action="{{ route('playlists.detach', ['id' => $t->id]) }}" method="POST"
                                        style="float: left; margin-right: 10px;" data-toggle="tooltip" data-placement="top"
                                ...
                                <span class="badge-user text-bold" style="float:right;">
                                    ...
                                    {{ $t->torrent->leechers }} /
                                    <i
                                        class="{{ config('other.font-awesome') }} fa-fw fa-check text-orange"></i>{{ $t->torrent->times_completed }}
                                </span>&nbsp;
                            ...
                            <div class="card_body">
                                <div class="body_poster">
                                    ...
                                    @if ($t->torrent->category->no_meta || $t->torrent->category->music_meta)
                                        <img src="https://via.placeholder.com/160x240" class="show-poster"
                                        ...
                                        <a href="{{ route('torrent', ['id' => $t->torrent->id]) }}">{{ $t->torrent->name }}
                                            ...
                                            @if ($t->torrent->category->game_meta && isset($meta) && $meta->first_release_date)
                                                <span class="text-bold text-pink">
                                                    {{ date('Y', strtotime($meta->first_release_date)) }}</span>
                                            @endif
                                        </a>
                                    </h3>
                                    ...
                                    @if ($t->torrent->anon == 1)
                                        <span
                                            ...
                                            @if (auth()->user()->id == $t->torrent->user->id || auth()->user()->group->is_modo)
                                                <a
                                                    href="{{ route('users.show', ['username' => $t->torrent->user->username]) }}">
                                        ...
                                        <a href="{{ route('users.show', ['username' => $t->torrent->user->username]) }}">
                                            <span class="badge-user text-bold"
                                                ...
                                                <i class="{{ $t->torrent->user->group->icon }}" data-toggle="tooltip"
                                                    title=""
                                                    data-original-title="{{ $t->torrent->user->group->name }}"></i>
                                                {{ $t->torrent->user->username }}
                                            </span>
                                        </a>
                                    @endif
