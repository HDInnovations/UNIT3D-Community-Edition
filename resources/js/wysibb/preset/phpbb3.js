    ...
    allButtons: {
        quote: {
            transform: {
                ...
                '<blockquote><div><cite>{AUTHOR} писал(а):</cite>{SELTEXT}</div></blockquote>':
                    '[quote="{AUTHOR}"]{SELTEXT}[/quote]',
            },
        },
        code: {
            transform: {
                '<dl class="codebox"><dt>Код: <a href="#">Выделить всё</a></dt><dd><code>{SELTEXT}</code></dd></dl>':
                    '[code]{SELTEXT}[/code]',
            },
        },
        bullist: {
                ...
                tabs: [
                    {
                        title: CURLANG.add_attach,
                        html:
                            '<div id="imguploader"> <form id="fupform" class="upload" action="{img_uploadurl}" method="post" enctype="multipart/form-data" target="fupload"><input type="hidden" name="iframe" value="1"/><input type="hidden" name="idarea" value="message" /><div class="fileupload"><input id="fileupl" class="file" type="file" name="img" /><button id="nicebtn" class="wbb-button">' +
                            CURLANG.modal_img_btn +
                            '</button> </div> </form> </div><iframe id="fupload" name="fupload" src="about:blank" frameborder="0" style="width:0px;height:0px;display:none"></iframe></div>',
                    },
                ],
                onLoad: fileModal,
            },
            transform: {
                '<div class="inline-attachment" numimg="{NUM[0-9]}" contenteditable="false"><dl class="file"><dt class="attach-image"><img src="./download/file.php?id={ID[0-9]}"></dt><dd>{ALT} Просмотров: 0</dd></dl></div>':
                    '[attachment={NUM[0-9]}]{ID[0-9]}: {ALT}[/attachment]',
                '<div class="inline-attachment" num="{NUM[0-9]}" contenteditable="false"><dl class="file"><dt><img src="./styles/prosilver/imageset/icon_topic_attach.gif" width="7" height="10" alt="" title=""><a class="postlink" href="./download/file.php?id={ID[0-9]}">{ALTFILE}</a></dt><dd>Скачиваний: 0</dd></dl></div>':
...
(function($) {
    ...
    $.wysibb.prototype.traceTextareaEvent = function(e) {
        ...
        if (
            ...
            if (data.indexOf('[attachment=') != -1) {
                ...
                let ext = $("input[name='attachment_data[" + num + "][real_filename]']")
                    .val()
                    .replace(/.*?\.(\w+)$/, '$1');
    ...
    $.fn.insertAttach = function(id, alt, isimg) {
        ...
        this.data('wbb').$txtArea.after(
            '<input type="hidden" name="attachment_data[' +
                num +
                '][attach_id]" value="' +
                id +
                '" /><input type="hidden" name="attachment_data[' +
                num +
                '][is_orphan]" value="1" /><input type="hidden" name="attachment_data[' +
                num +
                '][real_filename]" value="' +
                id +
                ':' +
                ...
                '" /><input type="hidden" name="attachment_data[' +
                num +
                '][attach_comment]" value="" />'
        );
        let data =
            isimg === true
                ? this.data('wbb').getCodeByCommand('attach', { id: id, num: num, alt: alt })
        ...
        this.$modal.find('#imguploader').dragfileupload({
            ...
            success: $.proxy(function(data) {
                ...
                if (data && data.status == 1) {
                    ...
                    let datastr =
                        data.isimg === true
                            ? this.getCodeByCommand('attach', { id: data.id, num: num, alt: data.alt })
