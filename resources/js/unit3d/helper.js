...
class uploadExtensionBuilder {
  removeDots (title) {
    ...
    if (title.includes(' REMUX')) {
      ...
      if (!title.includes('BluRay') && !(lowerTitle.includes(' dvd ') || lowerTitle.includes(' ntsc ') || lowerTitle.includes(' pal '))) {
    ...
    if ((title.includes('Blu-ray') || title.includes('BluRay')) && title.includes(' DV') && !title.includes('HDR')) {
    ...
    if (title.toLowerCase().includes('hybrid')) {
      ...
      title = title.replace(/(.*)(S\d{2}E\d{2}|S\d{2}|\d{4})( .*)(Hybrid)(.*)( \d{3,4}[ip] )/i, '$1$2$3 $5 Hybrid $6')
    }
    // Fix extra spaces
    title = title.replace(/ +/g, ' ')
  ...
  hook () {
    ...
    if (!name.value.trim() && !tmdb.value.trim()) {
      ...
      function success (data) {
        data = JSON.parse(data)
        if (release.type === 'Movie') {
          let tags = data.keywords.map(({
                                          name
                                        }) => name).join(', ')
          $('#autokeywords').val(tags)
        } else if (release.type === 'TV Show') {
          let tags = data.results.map(({
                                         name
                                       }) => name).join(', ')
          $('#autokeywords').val(tags)
        }
